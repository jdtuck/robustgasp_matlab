...
assert_close(pr.lower95, mu_ref + sqrt(var_ref)*rgasp_tinv(0.025, m.model.num_obs-m.model.q), ...
             1e-5, 'lower 95% bound');
assert_close(pr.upper95, mu_ref + sqrt(var_ref)*rgasp_tinv(0.975, m.model.num_obs-m.model.q), ...
             1e-5, 'upper 95% bound');
df = m.model.num_obs - m.model.q;
...
if mn.model.nugget <= 0 || ~isfinite(mn.model.nugget)
    error('nugget estimation failed: eta = %g', mn.model.nugget);
end
noise_var_hat = mn.model.nugget * mn.model.sigma2_hat;
...
% ----------------------------------------------------------------------
function [mu, v] = brute_force_predict(mi, xt)
m = mi.model;
